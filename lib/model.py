...
class PDModel(Model):
        ...
        for x in range(width):
            for y in range(height):
                if agent_type == 'mixed':
                    ...
                    type_str = agent_type
                if (x - 10.0) < 0.5:
                    agent = self.create_agent('simple')
                else:
                    if (x - 19.0) < 0.5:
                        agent = self.create_agent('tit_for_tat')
                    else:
    ...
    def step(self):
        ...
        if self.schedule.steps >= self.num_substeps:
            with open('one_simple_one_tat_others_tat_others_NN.txt', "a+") as f:
                f.write(str(self.total_scores / len(self.agents)))
                f.write('\n')
